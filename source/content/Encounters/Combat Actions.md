...
2. *Determine Opponent’s AC*
	- The Referee announces the opponents’s **Armor Class (AC)**.  
	- AC = **14 (Base AC) – opponents’s Agility Bonus**.  
	- Modify further if the opponent is armed or equipped:
		- Subtract **Weapon Skill** vs melee attacks.
		- Apply modifiers for a **shield** or **off-hand weapon**.
3. *Adjust AC for the Attacker*
	- Add the attacker’s bonuses to the opponent’s AC:
	- **Melee attacks**: add attacker’s **Weapon Skill + Strength Bonus**.
	- **Ranged attacks**: add attacker’s **Ballistic Skill + Agility Bonus**.
4. *Roll to Hit*
	- The attacking player rolls **1d20**.  
	- Compare the result to the adjusted AC and the opponent’s **Armor Rating (AR)**:  
		- **Result ≤ (AC – AR):** Full hit (successful attack).
		- **Result > (AC – AR) but ≤ AC:** Glancing hit (always deals **1 Damage**).
		- **Result > AC:** Miss. 
5. *Calculate Damage*
	- On a **successful attack** (full hit): Damage = **unmodified d20 roll + weapon’s Damage**.
	- On a **glancing hit:** Damage = **1**. 

Damage rules are detailed in [[Damage]] Section.

---

*Example situation: Kurt is a 4th-level Mercenary with Weapon Skill 4, +1 Strength Bonus, and a Messer (Medium Weapon, 1d8 damage). Hans is a 2nd-level Mercenary with Weapon Skill 2, +2 Agility Bonus, and Chainmail (Armor Rating 4). Hans’s Armor Class is calculated as 14 minus his Agility Bonus (2) for 12, then reduced by his Weapon Skill (2) against melee, for a final AC of 10. Kurt’s attack modifies this AC by adding his own Weapon Skill (4) and Strength Bonus (+1), raising the target number to 15. Kurt rolls a d20: if the result is 11 or less (15 − AR 4), it is a full hit; if the result is between 12 and 15 it is a glancing hit for 1 damage; and if higher than 15 it is a miss.*  

- ***Roll = 6:** This is a full hit (6 ≤ 11). Kurt deals 6 + 1d8 damage, combining the force of his strike with the weapon’s damage.*  
- ***Roll = 14:** This is a glancing hit (between 12 and 15). The Messer connects but doesn’t break through Hans’s armor, inflicting only 1 damage.*  
- ***Roll = 18:** This is a miss (greater than 15). Kurt’s swing goes wide, and Hans avoids the attack completely.*
---
Some special conditions may modify the usual attack flow:

(***Note*:** In the following examples, “+X” or “−X to hit” indicates a modification applied to the opponent’s AC.)
...
If using a missile weapon, a character can decide to take a full Round to aim. The aiming character has no Agility modifiers to his AC. On the following round the aiming character receives a +2 to hit when firing on his action. The aiming time is in addition to normal reload times.
